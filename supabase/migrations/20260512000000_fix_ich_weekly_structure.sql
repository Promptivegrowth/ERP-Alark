...
--     - Desactiva los campos semanales viejos (no se borran: se preservan
--       sus valores históricos; simplemente no aparecen en el formulario).
--     - Inserta la nueva estructura como espejo PURO del reporte diario de
--       ICH (sin extras; ICH no maneja loncheras ni coffe break facturable
--       por la vía semanal — el coffe break del diario es un toggle aparte).
...
BEGIN
  ...
  -- 2. Insertar nueva estructura — espejo puro del reporte diario de ICH
  ...
  -- 3. Por si una corrida anterior dejó estos extras activos, desactivarlos
  UPDATE reporte_semanal_campos SET activo = false
   WHERE comedor_id = id_ich AND nombre_campo IN ('COFFE BREAK', 'LONCHERAS DOBLES');
END $$;

...
export default function KardexSnacksPage() {
    ...
    const form = useForm<KardexForm>({
        defaultValues: {
            semana: new Date().toISOString().split('T')[0], // Using current date to represent the week
            items: []
        }
    });
    ...
    return (
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 pb-24">
            <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold tracking-tight">Kardex de Snacks</h2>
                <Input
                    type="date"
                    className="w-48"
                            ...
                            {fields.map((field, idx) => {
                                ...
                                return (
                                    <TableRow key={field.id}>
                                        ...
                                        <TableCell>
                                            <Input type="number" min="0" className="w-20" {...form.register(`items.${idx}.ingreso_semanal`, { valueAsNumber: true })} />
                                        </TableCell>
                                        <TableCell>
                                            <Input type="number" min="0" className="w-20" {...form.register(`items.${idx}.stock_anterior`, { valueAsNumber: true })} />
                                        </TableCell>
                                        <TableCell className="bg-zinc-50 dark:bg-zinc-900 font-bold">{saldo}</TableCell>
                                        <TableCell>
                                            <Input type="number" min="0" className="w-20" {...form.register(`items.${idx}.ventas_credito`, { valueAsNumber: true })} />
                                        </TableCell>
                                        <TableCell>
                                            <Input type="number" min="0" className="w-20" {...form.register(`items.${idx}.ventas_contado`, { valueAsNumber: true })} />
                                        </TableCell>
                                        <TableCell className="bg-zinc-50 dark:bg-zinc-900 font-bold text-blue-600">{tVentas}</TableCell>
                                        <TableCell>
                                            <Input type="number" min="0" className="w-20 border-emerald-500" {...form.register(`items.${idx}.stock_fisico`, { valueAsNumber: true })} />
                                        </TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell>
                                            <Input placeholder="Opcional..." {...form.register(`items.${idx}.observacion`)} />
                                        </TableCell>
                                    </TableRow>
                                )
                            })}
            ...
            <div className="fixed bottom-0 left-0 lg:left-64 right-0 bg-white dark:bg-zinc-950 border-t p-4 px-6 flex justify-end shadow-md z-10 text-sm">
                <Button size="lg" type="submit" disabled={isSubmitting}>
                    {isSubmitting ? 'Guardando...' : 'Guardar Kardex de Snacks'}
                </Button>
            </div>
        </form>
    );
